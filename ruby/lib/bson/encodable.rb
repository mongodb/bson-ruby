...
module BSON
  ...
  module Encodable
    ...
    def encode_bson_with_placeholder(encoded = ''.force_encoding(BINARY))
      ...
      yield(encoded)
      encoded << NULL_BYTE
      encoded[pos, 4] = (encoded.bytesize - pos).to_bson_int32
      encoded
    end
    ...
    def encode_string_with_placeholder(encoded = ''.force_encoding(BINARY))
      ...
      yield(encoded)
      encoded << NULL_BYTE
      encoded[pos, 4] = (encoded.bytesize - pos - 4).to_bson_int32
      encoded
    end
    ...
    def encode_binary_data_with_placeholder(encoded = ''.force_encoding(BINARY))
      ...
      encoded << PLACEHOLDER
      yield(encoded)
      encoded[pos, 4] = (encoded.bytesize - pos - 4 - 1).to_bson_int32
      encoded
    end
  end
end
